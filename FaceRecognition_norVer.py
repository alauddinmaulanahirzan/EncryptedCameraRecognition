...
def encodeImg():
    ...
    for imagedata in imageset:
        ...
        boxes = face_recognition.face_locations(rgb, model="hog")  # atau "hog" / "cnn"
        # Pemrosesan Wajah
    ...
    f = open("encodedset_nor", "wb")
    f.write(pickle.dumps(data_image))
    f.close()
## Encoding ##

## Recognition ##
def faceRecognition():
    image_data = pickle.loads(open("encodedset_nor", "rb").read())
...
def retrieveData():
    ...
    try:
        ...
        sql_retrieve = """SELECT * from imageset_normal"""
...
def main():
    ...
    encoded_file = os.path.exists("encodedset_nor")
    if(encoded_file == False):
        ...
        if(result == True):
            print("=> Dataset Sukses Diunduh. Encoding Data:")
            encodeImg()
        else:
            print("=> Membatalkan ...")
            sys.exit("")
